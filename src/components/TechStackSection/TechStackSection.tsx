...
const TechStackSection = () => {
  return (
    <div className="w-full py-6 border-t-2 border-b-2 border-[#2B1004]/20 dark:border-[#FFF2EF]/20 bg-[#2B1004]/[0.02] dark:bg-[#FFF2EF]/[0.02] flex flex-col items-center justify-center overflow-hidden">
      <motion.div 
        initial={{ opacity: 0 }}
      ...
      >
        ...
        <div className="flex w-max animate-[marquee_30s_linear_infinite] whitespace-nowrap items-center hover:[animation-play-state:paused] py-1">
          {[...technologies, ...technologies, ...technologies].map((tech, i) => (
            <div 
              key={i} 
              className="mx-3 px-5 py-2.5 rounded-full border border-[#2B1004]/20 dark:border-[#FFF2EF]/20 bg-background/80 text-foreground font-medium text-sm flex items-center gap-3 transition-all hover:scale-105 hover:border-primary/50 hover:bg-foreground/5 cursor-default shadow-sm group shrink-0"
            >
              <img 
                src={tech.icon} 
                alt={tech.name} 
